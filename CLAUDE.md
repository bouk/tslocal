...
- **Rust**: never use `.unwrap()` or `.expect()` — always propagate errors with `?` or handle them explicitly
- `rust/src/types.rs`, `ts/src/types.ts`, and `python/src/tslocal/_generated_types.py` are generated by `cmd/typegen` — never edit them manually, fix the generator instead
